package com.github.open.maven.plugin;
...
public class MsgBundleMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        try {
            // 1.创建包路径
            FileUtil.createPackage(targetPackage);

            // 2.创建java文件
            bw = FileUtil.createBlankJavaFile(targetPackage + "/" +  className + ".java");

            // 3.写入消息内容
            FileUtil.writeMsgJavaFile(bundleProps, templateProps, bw, sourcePackage, className, codeHead);
